...
import queue
import yappi
from splitpoint_MTDbi import SplitPoint
import sys
...
Score_Entry = namedtuple('Score_Entry', 'lower upper move depth')
...
class Searcher():
    ...
    def search(self, board, alpha, beta, depth, threadID, history):    
        ...
        if score_Entry is not None:
            ...
            if score_Entry.depth >= depth:
                ...
                if score_Entry.lower >= beta:
                    return score_Entry.lower
                if score_Entry.upper <= alpha:
                    return score_Entry.upper
        ...
        while moveList:
            ...
            if depth >=1 and len >2 and self.ThreadAccess.acquire(False):
                ...
                if(IDs):
                    t_split_start = time()
                    s = SplitPoint(board,moveList,depth,alpha,beta,score, threadID, bestmove, history)
                    self.split(s, IDs, t_split_start)
                    self.stabilitycheck(score, alpha, beta)
                    if s.score >= alpha:
                        self.TT_Score[TTkey] = Score_Entry(s.score, beta, s.bestmove, depth)
                    else:
                        self.TT_Score[TTkey] = Score_Entry(alpha, s.score, s.bestmove, depth)
                    return s.score #??? should be right, just break should return the old alpha and disregard the splitsearch
                
                #print("lock released")
        self.stabilitycheck(best, alpha, beta)
        if best >= alpha:
            self.TT_Score[TTkey] = Score_Entry(best,beta, bestmove, depth)
        else:
            self.TT_Score[TTkey] = Score_Entry(alpha, best, bestmove, depth)
        return best
    ...
    def splitpoint_search(self, splitpoint, threadID):
        ...
        while splitpoint.moveList and not self.told_to_stop(threadID):
            ...
            score = -self.search(newBoard,-splitpoint.beta, -splitpoint.alpha,-splitpoint.score, splitpoint.depth -1, threadID, history)
            history.pop()
            newBoard.pop()
            splitpoint.boundsLock.acquire()
            if score > splitpoint.score:
                splitpoint.score = score
                splitpoint.bestmove = move
            splitpoint.boundsLock.release()
            if score >= splitpoint.gamma:
